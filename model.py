...
print(f"Training miklium-lm-mini ({readable_count} params) with NumPy...")
...
def forward(tokens):
    ...
    for i in range(n_layer):
        ...
        mask = np.triu(np.ones((T, T)), k=1) * -10000000000.0
        att = att + mask
...
def backward(tokens, logits, cache, x_final):
    ...
    for i in reversed(range(n_layer)):
        (x_norm1, ms1, sc1, q, k, v, p, y_attn_h, x_norm2, ms2, sc2, h, h_relu) = cache[
            i
        ]
        dy_mlp = dx
        ...
        dp = dy_attn_v @ v.transpose(0, 2, 1)
        dv = p.transpose(0, 2, 1) @ dy_attn_v
        datt = p * (dp - np.sum(dp * p, axis=-1, keepdims=True))
        datt /= head_dim**0.5
        dq = datt @ k
        dk = datt.transpose(0, 2, 1) @ q
...
(beta1, beta2, eps) = (0.9, 0.95, 1e-08)
num_steps = 5000
...
for step in range(num_steps):
    ...
    if (step + 1) % 10 == 0 or step == 0:
        ...
        print(
            f"step {steps_done:4d} / {num_steps:4d} | loss {step_loss:.4f} | ETA: {eta_fmt}",
            end="\r",
        )
os.makedirs("website", exist_ok=True)
save_path = f"website/miklium-lm-mini_{readable_count}.miklium_model"
model_data = {
    "vocab": uchars,
...
def generate(prompt, length=500, temp=0.5):
    ...
    for _ in range(length):
        ...
        logits_last = logits[-1] / (temp + 1e-06)
